resource "azurerm_public_ip" "aadc_static" {
...
resource "azurerm_network_interface" "aadc_nic" {
  ...
  resource_group_name = var.resource_group_name
  ip_configuration {
    name                          = "aadc-ip-configuration"
    subnet_id                     = var.subnet_id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.aadc_static.id
  }
}
...
resource "azurerm_network_interface_security_group_association" "aadc_nic_association" {
  network_interface_id      = azurerm_network_interface.aadc_nic.id
...
resource "azurerm_virtual_machine_extension" "wait_for_domain_to_provision" {
  name                 = "SetupAzureADConnectServer"
  ...
  settings             = <<SETTINGS
  {
    "commandToExecute": "powershell.exe -Command \"${local.aadc_install_command}; ${local.install_rsat_command}; while (!(Test-Connection -ComputerName ${var.active_directory_domain_name} -Count 1 -Quiet) -and ($retryCount++ -le 360)) { Start-Sleep 10 } \""
  }
SETTINGS
}
